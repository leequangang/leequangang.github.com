...
网上给的办法是：
- 1、（我用的次方法）将dvi转成ps，再将ps转成pdf，也就是将`.bat`文件中的`dvipdfm lqg.dvi`替换成  
...
- 2、Rendering: colorspace要是彩色图片就选RGB color，灰色的就选black and white(其实就是灰度图像)，grayscale不知道什么用；Resolution（dpi）即图像分辨率（每英寸像素点数）600dpi就很高了；其他默认
